...
function addSeg(preset){
  ...
  segs.push({id:id,city:preset?(preset.city||''):'',hotel:preset?(preset.hotel||''):'',
    ci:preset?preset.ci:fmtDate(today),co:preset?preset.co:fmtDate(defCoD),
...
function renderSeg(id){
  ...
  for(var ri=0;ri<seg.rooms.length;ri++){
    ...
    for(var ci=0;ci<r.children.length;ci++){
      childHTML+='<div class="child-tag"><span>Child '+(ci+1)+' age</span>'
        ...
        +'<button class="rm cage-rm" data-seg="'+id+'" data-room="'+r.id+'" data-cidx="'+ci+'">×</button>'
        +'</div>';
    }
    roomsHTML+='<div class="room-card" id="room-'+id+'-'+r.id+'">'
      +'<div class="room-hdr"><span class="room-lbl">Room '+(ri+1)+'</span>'
      +(seg.rooms.length>1?'<button class="x-btn room-rm" data-seg="'+id+'" data-room="'+r.id+'">×</button>':'')
      +'</div>'
  ...
  el.innerHTML=''
    ...
    +'</div>'
    +(canRm?'<button class="x-btn seg-rm" data-seg="'+id+'">×</button>':'')
    +'</div>'
    +'<div class="slbl">City &amp; hotel</div>'
    +'<div class="g2" style="margin-bottom:12px;">'
    ...
    +'</select>'
    +'<div class="slbl">Rooms &amp; guests</div>'
