...
# ── Configuration (override via environment variables on Railway) ────────────
...
REPORT_FREQ      = os.environ.get("REPORT_FREQ", "every2days")   # daily|every2days|weekdays|weekly
...
# ── Default itinerary (overridden per-user when hosted) ──────────────────────
DEFAULT_ITINERARY = [
    {
...
def get_itinerary_for_user(user=None):
    if user:
        ...
        return profile.get("segs", DEFAULT_ITINERARY)
        ...
        try:
            resp = requests.get(
                ...
                params={"location": f"{lat},{lng}", "radius": 600,
                        "keyword": kw, "type": "restaurant", "key": PLACES_API_KEY},
                timeout=10
            )
...
def room_lines(rooms):
    ...
    for r in rooms:
        ...
        if r["children"]:
            ...
            s += f", {len(r['children'])} child{'ren' if len(r['children']) > 1 else ''} " \
                 f"(age{'s' if len(r['children']) > 1 else ''}: {ages})"
        if r.get("notes"):
            s += f" — {r['notes']}"
        out.append(s)
    return out
...
def build_email(report, is_alert=False, alert_details=None):
    ...
    for seg in report:
        ...
        if all_kids:
            guest_str += f", {len(all_kids)} child{'ren' if len(all_kids) > 1 else ''} " \
                         f"(ages: {', '.join(str(a) for a in all_kids)})"
        sections += f"""
        ...
        for h in seg.get("hotels", []):
            ...
            grand = h["price"] * seg["nights"] * len(seg["rooms"])
    ...
    if is_alert and alert_details:
        banner = f'<div style="background:#FCEBEB;border-radius:8px;padding:12px;' \
                 f'margin-bottom:20px;color:#791F1F;font-weight:500">{alert_details}</div>'

    tag = "⚠ PRICE ALERT" if is_alert else "Hotel Report"
    return f"""
    <html>
    <body style='font-family:sans-serif;max-width:640px;margin:auto;padding:24px;color:#222'>
